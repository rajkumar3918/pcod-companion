...
export default function SignupScreen({ onGoToLogin }) {
  ...
  return (
    <div style={{ padding: "40px 24px", display: "flex", flexDirection: "column", height: "100%", justifyContent: "center" }}>
      ...
      <p style={{ fontFamily: serif, fontWeight: 700, fontSize: 24, color: C.ink, margin: "0 0 4px", textAlign: "center" }}>Create your account</p>
      <p style={{ fontFamily: sans, fontSize: 13, color: C.muted, margin: "0 0 28px", textAlign: "center" }}>Your data stays private to your account</p>
      ...
      <form onSubmit={submit}>
        ...
        <input type="email" required placeholder="Email" value={email} onChange={(e) => setEmail(e.target.value)}
          style={{ width: "100%", boxSizing: "border-box", padding: "12px 14px", borderRadius: 12, border: `1.5px solid ${C.line}`, fontFamily: sans, fontSize: 14, marginBottom: 10 }} />
        <input type="password" required minLength={6} placeholder="Password (6+ characters)" value={password} onChange={(e) => setPassword(e.target.value)}
      ...
      <button onClick={onGoToLogin} style={{ border: "none", background: "none", color: C.muted, fontFamily: sans, fontSize: 12.5, marginTop: 16, cursor: "pointer" }}>
        Already have an account? <span style={{ color: C.forest, fontWeight: 700 }}>Sign in</span>
      </button>
    </div>
  );
}
