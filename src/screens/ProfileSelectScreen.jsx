...
export default function ProfileSelectScreen() {
  ...
  return (
    <div style={{ padding: "40px 24px", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", height: "100%" }}>
      ...
      <p style={{ fontFamily: sans, fontSize: 12, color: C.muted, margin: "0 0 24px" }}>Up to {MAX_PROFILES} profiles on this device</p>

      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 18, width: "100%", marginBottom: 6, justifyItems: "center" }}>
        {profiles.map((p) => (
          <button key={p.id} onClick={() => setProfile(p)} style={{ border: "none", background: "none", cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", gap: 8 }}>
            <div style={{ width: 68, height: 68, borderRadius: 18, background: p.color || C.forest, display: "flex", alignItems: "center", justifyContent: "center" }}>
        ...
        {profiles.length < MAX_PROFILES && !adding && (
          <button onClick={() => setAdding(true)} style={{ border: "none", background: "none", cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", gap: 8 }}>
      ...
      {adding && (
        <form onSubmit={addProfile} style={{ width: "100%", marginTop: 18 }}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <button type="button" onClick={() => { setAdding(false); setError(""); }} style={{ flex: 1, border: `1.5px solid ${C.line}`, background: C.white, color: C.muted, borderRadius: 12, padding: "12px", fontFamily: sans, fontWeight: 700, fontSize: 13.5, cursor: "pointer" }}>
              Cancel
            </button>
            <button type="submit" style={{ flex: 1.4, border: "none", background: C.forest, color: C.white, borderRadius: 12, padding: "12px", fontFamily: sans, fontWeight: 700, fontSize: 13.5, cursor: "pointer" }}>
              Add profile
            </button>
          </div>
        </form>
