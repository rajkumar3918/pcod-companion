...
export default function MealCard({ slot, condition, selectedName, onChange }) {
  const [expanded, setExpanded] = useState(false);
  const [sheetOpen, setSheetOpen] = useState(false);
  const options = slot.options[condition] || slot.options.normal;
  const current = getCurrentMeal(slot, condition, { [slot.id]: selectedName });
  ...
  return (
    <div style={{ background: C.white, borderRadius: 20, border: `1px solid ${C.line}`, marginBottom: 14, overflow: "hidden", position: "relative", boxShadow: "0 2px 10px rgba(40,39,31,0.05)" }}>
      ...
      <div style={{ padding: "14px 16px" }}>
        ...
        {expanded && (
          <div style={{ background: slot.bg, borderRadius: 12, padding: "10px 12px", marginBottom: 10 }}>
